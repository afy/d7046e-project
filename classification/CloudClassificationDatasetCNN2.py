...
class CloudClassificationDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        cli=5
        img_disp = torch.from_numpy(np.clip((imagergb-np.percentile(imagergb,cli))/(np.percentile(imagergb,100-cli)-np.percentile(imagergb,cli)),0, 1)[:20,:20,:]).permute(2,0,1)

        if self.transform is not None:
            image = self.transform(image)
            image  = v2.Normalize(mean=[0.485, 0.456, 0.406, 0.485, 0.456, 0.406, 0.485, 0.456], std=[0.229, 0.224, 0.225, 0.229, 0.224, 0.225, 0.229, 0.224])(image)
