...
class CloudDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        mask=mask-((mask==2).to(torch.int8)*1)
        mask=mask-((mask==3).to(torch.int8)*2)
        mask=mask-((mask==4).to(torch.int8)*2)
        mask=mask-((mask==5).to(torch.int8)*2)
        onehot_mask =F.one_hot((mask[0]).to(torch.int64),num_classes=4).permute(2,0,1)
